      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-8">
        ...
        <div className="relative aspect-[3/4] rounded-xl md:rounded-2xl overflow-hidden group shadow-lg">
          ...
          <button
            ...
            className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-1.5 md:p-2 rounded-full md:opacity-0 group-hover:opacity-100 transition-opacity"
          >
            <svg className="h-4 w-4 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            ...
            className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-1.5 md:p-2 rounded-full md:opacity-0 group-hover:opacity-100 transition-opacity"
          >
            <svg className="h-4 w-4 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {currentModel.images.map((_, idx) => (
              <button
                ...
                className={`h-1.5 md:h-2 rounded-full transition-all ${
                  idx === currentImageIndex ? "w-6 md:w-8 bg-white" : "w-1.5 md:w-2 bg-white/50"
                }`}
              />
            ))}
          </div>
          ...
          <div className="absolute top-3 left-3 md:top-4 md:left-4">
            <Badge className="bg-amber-600 text-white gap-1 text-xs md:text-sm py-1 px-2">
              ...
              <span className="hidden sm:inline">{getAwardBadge(currentModel.awardType)}</span>
              <span className="sm:hidden">{currentModel.awardType === 'day' ? 'Day' : currentModel.awardType === 'week' ? 'Week' : 'Month'}</span>
            </Badge>
          </div>
        </div>
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
                <svg className="h-6 w-6 md:h-8 md:w-8 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 01-.982-3.172M9.497 14.25a7.454 7.454 0 00.981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 007.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 002.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 012.916.52 6.003 6.003 0 01-5.395 4.972m0 0a6.726 6.726 0 01-2.749 1.35m0 0a6.772 6.772 0 01-3.044 0" />
                </svg>
                <div>
                  <p className="text-xs md:text-sm text-gray-600">Award Winner</p>
                  <p className="text-sm md:text-base lg:text-lg font-bold text-gray-900">{getAwardBadge(currentModel.awardType)}</p>
                </div>
              </div>
            ...
            {models.map((model, idx) => (
              <button
                ...
                className={`relative aspect-square rounded-md md:rounded-lg overflow-hidden border-2 transition-all ${
                  idx === currentIndex ? "border-amber-600 scale-105 shadow-md" : "border-transparent hover:border-gray-300"
                }`}
              >
                <Image
                  src={model.avatar}
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-1 md:p-2">
                  <p className="text-white text-[10px] md:text-xs font-medium text-center">{model.name}</p>
                </div>
              </button>
            ))}
          </div>
